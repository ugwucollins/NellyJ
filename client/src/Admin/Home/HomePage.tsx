import { BiDollar } from "react-icons/bi";
import { GrDeliver } from "react-icons/gr";
import { MdEventAvailable, MdProductionQuantityLimits } from "react-icons/md";
import { PiPlus } from "react-icons/pi";
import { adminPath } from "../../context/UserContext";
import type { AddCardProp, CardProp } from "../../seller/Context/Types";
import { UserAdminAuth } from "../context/AdminContext";
import { RealValues } from "../../seller/Context/RealValues";
import AddCard from "../../seller/Context/AddCard";
import Card from "../../seller/Context/Card";
import HeaderProp from "../../context/HeaderProp";
import TwoLevelPieChart from "../charts/PieChart";
import LineChartExample from "../charts/LineChart";

const HomePage = () => {
  return (
    <div className="w-full">
      <div className="w-full sticky top-0 z-[1]">
        <HeaderProp
          LinkText1="Home"
          LinkText2="dashboard"
          AnText="Admin Dashboard"
          LinkPath={adminPath}
        />
      </div>
      <div className="px-4 py-5">
        <HomePageDisplay />
      </div>
    </div>
  );
};

export default HomePage;
const HomePageDisplay = () => {
  const { admin }: any = UserAdminAuth();
  const { product, orders, sales, customers, events, sellers } = RealValues();
  const RealArray: any = [sales, orders, events, product, customers, sellers];

  return (
    <div>
      <div className=" mb-4 py-2 px-2">
        <h1 className="font-bold text-xl">Dashboard Overview</h1>
        <h1 className="opacity-90 text-sm font-semibold">
          Welcome {admin && admin.email.split("@gmail.com")}!
        </h1>
      </div>

      <div className="flex flex-wrap justify-around max-md:justify-center gap-5 my-5">
        {CardArray.map((item: CardProp, index: number | any) => (
          <Card
            key={index}
            index={index}
            Title={item.Title}
            icon={item.icon}
            value={RealArray[index]}
            color={item.color}
            pen={item.pen}
            path={item.path}
          />
        ))}
      </div>
      <div className="w-full flex justify-center max-[1100px]:flex-col items-center py-5 bg-slate-50/50 dark:text-black dark:bg-transparent rounded-xl shadow-md max-[1100px]:gap-6">
        <LineChartExample />
        <TwoLevelPieChart />
      </div>
      <div className="flex pb-4 flex-wrap gap-4 justify-around max-md:justify-center px-4">
        {CardArrayLong.map((item: AddCardProp, index) => {
          const even = index % 2 === 0;
          return (
            <AddCard
              index={index}
              key={item.Title}
              Title={item.Title}
              icon={item.icon}
              className={
                even
                  ? "w-full flex justify-end py-1"
                  : "w-full flex justify-start py-1"
              }
              text={item.text}
              color={item.color}
              path={item.path}
              btnText={item.btnText}
            />
          );
        })}
      </div>
    </div>
  );
};

export const CardArrayLong: AddCardProp[] = [
  {
    Title: "Add New Product",
    icon: <PiPlus />,
    text: "Quickly add a new product to your store inventory",
    color: "purple",
    btnText: "Add Product",
    path: adminPath + "/addproduct",
  },
  {
    Title: "See All Product",
    icon: <MdProductionQuantityLimits />,
    text: "Quickly List of all the new product to your store inventory",
    color: "#854d0e",
    btnText: "View Product",
    path: adminPath + "/listproduct",
  },
  {
    Title: "Event Booked",
    icon: <MdEventAvailable />,
    text: "Quickly see all the new booked Events in your store inventory",
    color: "#9a3412",
    btnText: "Book Event",
    path: adminPath + "/events",
  },
  {
    Title: "See All Orders",
    icon: <GrDeliver />,
    text: "Quickly List of all the new product to your store inventory",
    color: "#059669",
    btnText: "View Orders",
    path: adminPath + "/orders",
  },
];
export const CardArray: CardProp[] = [
  {
    Title: "Total Sales",
    icon: <BiDollar />,
    value: " $24,789",
    color: "green",
    pen: "+12.5 from past last month",
    path: adminPath,
  },
  {
    Title: "Total Orders",
    icon: <GrDeliver />,
    value: " 120",
    color: "yellow",
    pen: "+20 from past last week",
    path: adminPath + "/orders",
  },
  {
    Title: "Booked Events",
    icon: <MdEventAvailable />,
    value: "120",
    color: "#059669",
    pen: "+10 from past last week",
    path: adminPath + "/events",
  },
  {
    Title: "All Products",
    icon: <MdProductionQuantityLimits />,
    value: "1299",
    color: "Orange",
    pen: "+12 Total New product",
    path: adminPath + "/listproduct",
  },
  // {
  //   Title: "Total Customers",
  //   icon: <MdProductionQuantityLimits />,
  //   value: "1299",
  //   color: "blue",
  //   pen: "20 Total New User/customers",
  //   path: adminPath + "/users",
  // },
  // {
  //   Title: "Total Sellers",
  //   icon: <BiUserPlus />,
  //   value: "1299",
  //   color: "#9a3412",
  //   pen: "5 Total New Sellers",
  //   path: sellerPath,
  // },
];
